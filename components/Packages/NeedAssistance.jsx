...
const NeedAssistance = () => {
    ...
    const onSubmit = async (e)=>{
        e.preventDefault();
        let response = await fetch('/api/query/add', {
            method:'POST',
            headers:{
    ...
    return (
        ...
        <Toaster position='top-right'/>
            <div className="w-full border text-white justify-center items-center  border-red-500 rounded-md flex gap-2 flex-col p-4 bg-red-500">
                ...
                <Button   style={{ color: 'red', background:'white', borderColor:'white' }}  onClick={() => { setIsCall(true); handleOpen(); setData({'call':true}) }} variant="outlined" startIcon={<CallIcon />} className='mt-2 bg-white text-red-500 border-white w-full focus:bg-white focus:border-white focus:text-red-600 hover:bg-white hover:border-white hover:text-red-600 '>
                    Request a Callback
                </Button>
                <Button   style={{ color: 'red', background:'white', borderColor:'white' }}  variant="outlined"  onClick={() => { setIsCall(false); handleOpen();  setData({'call':false})  }}  startIcon={<MailIcon />} className='mt-2 bg-white text-red-500 border-white w-full focus:bg-white focus:border-white focus:text-red-600 hover:bg-white hover:border-white hover:text-red-600'>
                    Send Email Request
                </Button>
            </div>
            <Modal
            ...
            >
                <Fade in={open}>
                    <Box sx={style} className="flex flex-col ">
                        ...
                        <form  onSubmit={onSubmit} className="p-4 pt-0 flex flex-col gap-2 ">
                            <TextField required onChange={handleOnChange} id="name" name='name' label="Name" className='w-full' variant="standard" />
                            <TextField required onChange={handleOnChange} id="email" name='email' label="Email" className='w-full' variant="standard" />
                            <TextField required onChange={handleOnChange} id="phone" name='phone' label="Phone" className='w-full' variant="standard" />
