...
const QuoteList = ({ orders, fetchAllOrders }) => {
    ...
    const fetchQuote = async () => {
        const response = await fetch('/api/quote/getall', {
            method: 'POST',
            headers: {
    ...
    const updateSeenStatus = async (id) => {
        const response = await fetch('/api/quote/updateseenstatus', {
            method: 'POST',
            headers: {
                'content-type': 'application/json',
            },
        ...
        <List className='-mt-4'>
            {
                data && data.length > 0 && data.map((item, index) => {
                    return <ListItem key={index} disablePadding className='border-b'>
                        <ListItemButton onClick={()=>{updateSeenStatus(item._id)}} component="a" href={`/admin/quote/${item._id}`} className='w-full'>
